...
- 現在の運用では、バックエンドへの同時アクセス数（並列度）は **5固定** としています。
- 理由はシンプルで、同時実行数を増やしすぎると、API 側の制限に引っかかって `429`（リクエスト過多）や一時的な失敗が増えやすくなるためです。
- 並列度5は「速度」と「失敗しにくさ」のバランスが最も安定しやすい設定として採用しています。
- そのため、現時点ではむやみに並列度を上げず、まずは安定運用を優先してください。

## 将来課題（優先順位）

- 将来的に処理性能を改善する場合でも、**先に並列度の設定値を変更するのではなく、レート制御キューを追加することを優先** します。
- 先にキューで流量を平準化してから、必要があれば並列度の再調整を検討する方針です。
